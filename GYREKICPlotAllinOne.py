...
if __name__=="__main__":
    ...
    for num,Star in enumerate(Stars):
        ...
        plt.subplot(sp[num])
        plt.plot(teff,logg,'.k',markersize=0.2)
        ...
        for i in ModelDict:
            ...
            plt.plot(trz.teff,trz.logg,'ow',markeredgecolor='black',markersize=4) #unconstrained
            
        if Star == 'B3':
            plt.xlim([24.60,22.60])
            plt.ylim([5.1, 5.6])  
            obsTeff = 23.540
            obsTefferr = 0.210
            obslogg = 5.311
        ...
        #plt.xlabel('T$\mathregular{_{eff}}$ [kK]')
        plt.ylabel(r'$\log$(g/cm s$^{-2}$)',size= 15)
        if Star =='B3':
            plt.text(23.1,5.55,r'KIC$\,2569576$',size= 15)
        if Star =='B4':
            plt.text(24.8,5.65,r'KIC$\,2438324$',size= 15)

        #'KIC2991403','KIC11179657'
        if Star =='KIC2991403':
            plt.text(26.55,5.76,r'KIC$\,2991403$',size= 15)
        if Star =='KIC11179657':
            plt.text(24.6,5.06,r'KIC$\,11179657$',size= 15)
        
        #ax1.invert_xaxis()
        ax1.invert_yaxis()    
    
    #plt.text(25,5.05,r'$\log$(g/cm s$^{-2}$)', rotation=90, va='center',size= 15)    
    plt.xlabel('T$\mathregular{_{eff}}$ [kK]',size= 15)
    plt.savefig(f'KIC_111_299_ConSolnZoom.png')
    plt.savefig(f'KIC_111_299_ConSolnZoom.eps')
    plt.show()
